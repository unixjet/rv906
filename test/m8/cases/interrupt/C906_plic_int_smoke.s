...
.global handler
handler:
      li x13, 0x10000004  # UART IER: deassert the level source
      li x12, 0           # BEFORE complete, else it re-fires after mret
      sb x12, 0(x13)
      nop
#//int cmplt
#//st id to claim_reg
...
init_prio:
      ...
      add x7, x5,x6
      li x8, 4
      sw x8, 0x1c(x7)   # source 7 prio: 0x4*7
...
#//assert UART irq source 7 (IER = THRE) -- LAST poke: the level
#//must rise after threshold-off so the trap lands on the wfi
#//(epc = wfi+4), not on a store's retire boundary (M6 pattern)
.global init_ip
init_ip:
      li x12, 2
