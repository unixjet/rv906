...
2. **D-side MMU port carries the PAGE NUMBER, not the byte VA (donor fact).** Donor: `aq_lsu_ag.v:1566` `assign lsu_mmu_va[51:0] = ag_pipe_addr[63:12]` (a 52-bit *output* of the LSU, `aq_lsu_ag.v:271`), response `input [27:0] mmu_lsu_pa` (`aq_lsu_ag.v:201`), PA reassembled by the requester as `ag_pipe_pa = {mmu_pa, ag_pipe_addr[11:0]}` (`aq_lsu_ag.v:1446`). The I-side uses the SAME convention (`ifu_mmu_va = icache_rd_addr[63:12]`). The Task 6 crash-agent's `LSU.v` deviated by driving the byte VA and making MMU.v do the `>>12` — behaviorally equivalent, but not clone-faithful. **Resolution (fixup commit after `1acdf58`): the shift lives in LSU.v** (`assign lsu_mmu_va = ag_addr[12 +: MMU_VA_WIDTH];  // = ag_addr[63:12]`), MMU.v is a pure page-in/page-out identity on both ports (`pa = <port>_mmu_va[27:0]`), and both benches + plan contract 2 were realigned to the page-number convention. (An intermediate committed attempt had put the `>>12` in MMU.v and switched the benches to a byte-VA convention — that direction was overturned on the user's clone-faithfulness review; the final state is donor-faithful.)
3. **Bench convention** (`mmu_tb.cpp`): realigned to the page-number convention (feeds the VPN into the DTLB port, mirroring `lsu_tb.cpp`'s real client and ICache's own driving).
